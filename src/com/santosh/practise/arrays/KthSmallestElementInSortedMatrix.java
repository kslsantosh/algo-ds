...
public class KthSmallestElementInSortedMatrix {
    ...
    private static int getKthSmallestElementFromSortedMartix(int[][] matrix, int k) {
        ...
        int removedCount = 0;
        PriorityQueue<HeapNode> minHeap = new PriorityQueue<HeapNode>((o1, o2) -> o1.data - o2.data);
        minHeap.add(new HeapNode(matrix[0][0], 0, 0));
        visited[0][0] = 1;
        HeapNode lastRemovedElement = null;
        while(removedCount != k) {
            HeapNode currentMinimumElement = minHeap.poll();
            ...
            if(isElementOnRightValidAndNotVisited) {
                minHeap.add(new HeapNode(matrix[currentRow+1][currentColumn], currentRow+1, currentColumn));
            ...
            if(isElementOnBottomValidAndNotVisited) {
                minHeap.add(new HeapNode(matrix[currentRow][currentColumn+1], currentRow, currentColumn+1));
    ...
    private static boolean isElementOnRightValid(HeapNode currentMinimumElement, int numRows, int[][] visited) {
    ...
    private static boolean isElementOnBottomValid(HeapNode currentMinimumElement, int numCols, int[][] visited) {
